...
| 1%당 총 가시적 토큰 | 1.6M-2.1M | Output + Cache Read + Input 합산 |
...
캐시 문제(v2.1.89)는 v2.1.90-91에서 **수정**되었습니다. 하지만 5개의 추가 클라이언트 측(사용자 컴퓨터에서 작동하는 프로그램의) 버그와 서버 측 쿼터 변경은 여전히 남아 있습니다.
...
| **B2** Resume | `--resume`(이어쓰기) 명령이 이전 대화 전체를 캐시 없이 다시 전송 | 한 번 이어쓰기할 때마다 평소의 20배 비용이 발생 | **수정됨** | [01_BUGS.md](01_BUGS.md#bug-2--resume-cache-breakage-v2169) |
...
| 4월 3일 | 버그 5 발견 (200K budget cap). v2.1.91 벤치마크: 캐시 수정 확인, 5개 다른 버그 지속. [06_TEST-RESULTS-0403.md](06_TEST-RESULTS-0403.md) |
| 4월 4-5일 | cc-relay가 rate limit 헤더와 함께 3,702건 요청 캡처. 커뮤니티 분석 계속 |
