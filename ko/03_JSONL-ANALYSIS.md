...
*환경: 데이터셋 `ubuntu-1-stock` — Max 20x ($200/월), Opus 4.6 1M, v2.1.91, Linux (ubuntu-1), 네이티브 `~/.claude` (CC stock 모드). 이 섹션의 수치는 당시 스냅샷(JSONL 파일 1,735개, 약 1.0 GB)에서 계산되었으며 같은 데이터셋의 일부입니다. 현재 `ubuntu-1-stock` 코퍼스는 2,098개 파일(911 MB)까지 성장하였습니다 — 라벨 매트릭스와 조정 내역은 [14_DATA-SOURCES.md](14_DATA-SOURCES.md) 참조. 프록시 교차 대조 부분집합: 8,794건 요청, rate limit 헤더 포함 3,702건 (4월 4-6일). 전체 프록시 데이터셋(4월 1–16일, 38,996건 요청) 및 JSONL 대량 스캔(532개 파일 과거 스냅샷): [13_PROXY-DATA.md](13_PROXY-DATA.md).*
